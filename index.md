# Privacy Policy for FairOwe

**Effective date:** September 24, 2026  
**Developer:** Black and Blue

FairOwe is an Android expense-sharing and settlement app published by Black and Blue (“FairOwe,” “we,” “us,” or “our”). This policy explains what the app processes, what may leave your device, and the choices available to you.

FairOwe does not require an account. We do not sell personal data or show advertising.

## 1. Information processed on your device

FairOwe stores the information you enter—such as group and member names, expense descriptions, notes, amounts, categories, currencies, and settlement records—in the app’s private local storage on your Android device. It uses this information to calculate balances, display group activity, and record settlements.

FairOwe does not upload this expense content to Black and Blue, Firebase, Google Play, or RevenueCat.

If you use FairOwe Pro, you can explicitly export one group as a CSV file. The file is created in private cache storage and shared only through Android’s system share sheet with the app or recipient you choose. FairOwe does not automatically upload CSV exports.

## 2. Analytics and crash diagnostics

FairOwe uses Google Analytics for Firebase for limited app-usage measurement and Firebase Crashlytics for reliability monitoring and crash diagnosis. These services may process app interaction events, crash stack traces, relevant app state and device metadata, app version, and per-installation identifiers.

FairOwe does not attach group names, member names, expense descriptions, notes, amounts, RevenueCat customer identifiers, or other user-entered expense content to analytics events or crash reports. Advertising ID and Android ad-services identifier permissions are removed from the app.

Firebase data is encrypted in transit. Google’s current Firebase privacy and retention information is available at [Firebase Privacy and Security](https://firebase.google.com/support/privacy).

## 3. Optional purchases

FairOwe may offer:

- **FairOwe Pro**, a one-time non-consumable purchase that unlocks unlimited active groups and per-group CSV export while the dedicated Pro entitlement is active.
- **Student Developer Support**, optional repeatable consumable purchases at locally displayed store prices corresponding to US $1, $10, and $100 tiers. These purchases support the developer and may trigger a one-use in-app thank-you celebration. They do not unlock Pro, another paid feature, a badge, or a lasting entitlement. They are not charitable donations and are not tax-deductible.

Purchases are processed by Google Play and RevenueCat. They may process purchase history, transaction information, device and installation information, and an automatically generated anonymous App User ID. FairOwe does not receive or store full payment-card details and does not send RevenueCat your expense records, name, email address, advertising ID, or custom customer attributes.

Pending, canceled, failed, mismatched, unverified, refunded, or replayed transactions do not unlock paid access or create a new support acknowledgement. Pro can be restored where Google Play and RevenueCat verify the dedicated entitlement. Consumed support purchases are not restored as owned content.

## 4. Storage and retention

Local expense data remains in FairOwe until you delete the relevant record, clear the app’s storage, or uninstall the app. Android backup is disabled for FairOwe, so clearing storage or uninstalling can permanently remove local records and unredeemed acknowledgements.

FairOwe stores only hashed purchase-correlation values needed to prevent duplicate support acknowledgements; it does not persist raw Google Play purchase tokens, order identifiers, or RevenueCat transaction identifiers. Temporary CSV exports are cleaned up by age and count limits.

RevenueCat, Google Play, Firebase Analytics, and Firebase Crashlytics retain service data under their respective terms and retention policies.

## 5. Your choices and deletion {#delete-data}

You can delete expenses, settlements, and groups inside FairOwe. To remove all local FairOwe data, use Android **Settings → Apps → FairOwe → Storage → Clear storage**, or uninstall the app.

For a privacy or deletion request concerning RevenueCat purchase records, email **developer@blackandblue.co.in** with the subject **FairOwe privacy request**. Include the anonymous RevenueCat Support ID shown in FairOwe’s About screen if available. Do not send purchase tokens, order identifiers, payment-card details, or other sensitive information. Google may retain transaction records where required for security, accounting, or legal obligations.

## 6. Permissions

FairOwe uses Internet and network-state access for Google Play Billing, RevenueCat, Firebase Analytics, and Firebase Crashlytics. It uses Google Play Billing for optional purchases. It does not request access to your location, camera, contacts, notifications, photos, media, or general device storage.

## 7. Children

FairOwe is a general expense record-keeping tool and is not designed for children. We do not knowingly use the app to collect user-entered expense content from children. Parents or guardians may contact us using the address below.

## 8. Security

FairOwe relies on Android’s app sandbox and encrypted network connections used by its service providers. No storage or transmission method is completely risk-free. Protect access to your device and keep Android security updates current.

## 9. Changes

We may update this policy when FairOwe, its service providers, or legal requirements change. The current version will show its effective date above. Material changes will be reflected in the app or store listing when appropriate.

## 10. Contact

- **Developer:** Black and Blue
- **Email:** developer@blackandblue.co.in
- **Privacy page:** https://fairowe.blackandblue.co.in/

Users in India may send privacy questions or complaints to the email address above. This contact statement does not designate a statutory grievance officer or publish unverified legal or postal details.
